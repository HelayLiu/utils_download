...
from .var_types import AllIRVarType, NonSSAVarType
from .icfg import EdgeType
        ...
        if isinstance(convert_variable, Constant):
            if isinstance(convert_type, ElementaryType):
                if str(convert_type).startswith("uint") or str(convert_type).startswith("int"):
                    ...
                    try:
                        sym_res = BitVecVal(convert_variable.value, 256)
                    except Z3Exception:
                        print(convert_variable.value)
                        print(type(convert_variable))
                        exit(1)
